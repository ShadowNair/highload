...
* region — varchar(16)7. Алгоритмы
7.1 Алгоритм поиска и ранжирования каталога
Блок системы

Catalog Service, OpenSearch, storefront search.

Постановка задачи

Необходимо реализовать поиск по каталогу игр с поддержкой:

полнотекстового поиска по названию, разработчику и издателю;
фильтрации по жанрам, тегам, цене и статусу;
сортировки по релевантности, дате релиза и популярности;
выдачи результатов с малой задержкой при большом объеме read-трафика.
...
каталог содержит более 100 тыс. игр;
storefront является read-heavy контуром;
поиск должен работать быстрее, чем запросы напрямую к PostgreSQL по JSONB и строковым полям;
выдача должна поддерживать faceted navigation и фильтры без тяжелых join-операций.
Последовательность работы алгоритма
При изменении данных игры в PostgreSQL формируется событие обновления каталога.
Search Indexer асинхронно обновляет документ игры в OpenSearch.
В индекс записываются не только исходные поля игры, но и денормализованные атрибуты, необходимые для поисковой выдачи.
При пользовательском запросе Catalog Service выполняет запрос к OpenSearch.
Текстовые поля обрабатываются через inverted index и BM25-подобное ранжирование.
Фильтры по жанрам, тегам, цене и статусу применяются как filter-контур без влияния на scoring.
Результаты возвращаются в виде уже подготовленных карточек с основными атрибутами для storefront.
Используемые структуры данных

Используются:

inverted index по текстовым полям;
keyword-поля для фильтрации;
numeric/date-поля для сортировки;
денормализованный документ игры в games_search_index.
Рассматриваемые альтернативы

Рассматривались следующие варианты:

Поиск напрямую в PostgreSQL через ILIKE, GIN, tsvector.
Полнотекстовый поиск в PostgreSQL с дополнительной фильтрацией по JSONB.
Выделенный поисковый движок OpenSearch.
Обоснование выбора

Поиск в PostgreSQL подходит для небольших каталогов, но хуже масштабируется при интенсивном storefront-read трафике и сложных фильтрах.
OpenSearch выбран потому, что он лучше подходит для:

полнотекстового поиска;
фильтрации по множеству полей;
сортировки;
faceted navigation;
вынесения поисковой нагрузки из OLTP-контура.
Влияние на проектные решения

Выбранный алгоритм приводит к следующим решениям:

каталог хранится в PostgreSQL как источник истины;
в OpenSearch поддерживается отдельный денормализованный индекс;
в физической схеме появляются поля games_search_index;
межсервисное взаимодействие включает асинхронную переиндексацию через event stream;
нагрузка поиска переносится с PostgreSQL на OpenSearch.
7.2 Алгоритм доставки билдов и патчей через manifest и chunk download
Блок системы

Download / Depot Service, CDN, object storage.

Постановка задачи

Необходимо реализовать доставку крупных игровых файлов и обновлений таким образом, чтобы:

не передавать игру целиком при каждом обновлении;
эффективно использовать CDN и object storage;
уменьшать суммарный сетевой трафик;
обеспечивать устойчивую раздачу при высокой нагрузке.
Исходные данные, ограничения и требования

На вход алгоритм получает:

идентификатор игры;
идентификатор версии билда;
локальное состояние клиента;
список уже имеющихся на клиенте chunk-объектов.

Ограничения:

игровые билды и патчи имеют большой размер;
основной сетевой трафик проекта приходится именно на download/CDN-контур;
необходимо сохранять cache locality;
запросы должны хорошо масштабироваться на CDN и object storage.
Последовательность работы алгоритма
Для каждой версии игры формируется manifest.
Manifest содержит список chunk-объектов, их идентификаторы, размеры и контрольные суммы.
Клиент запрашивает manifest целевой версии.
Клиент сравнивает manifest с локальным состоянием.
Для отсутствующих или изменившихся chunk-объектов выполняется загрузка с CDN.
Chunk-объекты собираются в локальный билд игры.
После завершения сборки выполняется проверка контрольных сумм.
Используемые структуры данных

Используются:

manifest-файлы;
chunk-объекты фиксированного или квазипостоянного размера;
контрольные суммы;
metadata-объекты для depot/download.
Рассматриваемые альтернативы

Рассматривались следующие варианты:

Повторная загрузка игры целиком.
Rolling-diff подход, аналогичный rsync.
Manifest + chunk-based delivery.
Обоснование выбора

Повторная загрузка целиком слишком дорога по сети.
Rolling-diff сложнее в эксплуатации и хуже ложится на CDN/object storage модель.
Manifest + chunk download выбран как компромисс между:

простотой;
эффективностью кэширования;
хорошей поддержкой CDN;
снижением повторной передачи уже имеющихся данных.
Влияние на проектные решения

Выбранный алгоритм приводит к следующим решениям:

download-контур физически отделяется от storefront/API-контура;
используются отдельные backend-пулы под metadata и large binary objects;
в объектном хранилище данные организуются как depot/chunk objects;
CDN может кэшировать chunk-объекты независимо;
локальная балансировка download-контура строится вокруг cache-aware routing и origin shield.
7.3 Алгоритм синхронизации облачных сохранений
Блок системы

Cloud Save Service, ScyllaDB, S3 / MinIO.

Постановка задачи

Необходимо синхронизировать сохранения пользователя между устройствами так, чтобы:

быстро получать актуальную версию при запуске игры;
сохранять историю версий;
обнаруживать конфликты изменений;
не хранить бинарные blob-объекты в OLTP-БД.
Исходные данные, ограничения и требования
...
user_id;
game_id;
локальный файл сохранения;
метаданные локальной версии;
актуальные метаданные серверной версии.

Ограничения:

cloud save является частым пользовательским сценарием;
blob-файлы могут иметь заметный размер;
необходимо разделить горячий read-path “получить последнюю версию” и менее частый сценарий “посмотреть историю”.
Последовательность работы алгоритма
При запуске игры
Клиент запрашивает latest-метаданные по user_id и game_id.
Если локальная версия отсутствует или старее серверной, клиент скачивает актуальный blob.
Если локальная и серверная версия совпадают по checksum/version, загрузка не выполняется.
При завершении игры
Клиент вычисляет checksum нового сохранения.
Blob загружается в object storage.
В cloud_saves_versions_by_user_game создается новая версия.
В cloud_saves_latest_by_user_game обновляется указатель на актуальное состояние.
При обнаружении конфликта версий возможно сохранение обеих версий и последующее разрешение конфликта.
Используемые структуры данных

Используются:

таблица cloud_saves_latest_by_user_game;
таблица cloud_saves_versions_by_user_game;
объект cloud-saves/{user_id}/{game_id}/{version};
checksum;
version.
Рассматриваемые альтернативы
...
Хранить только одну актуальную версию.
Хранить только журнал версий без отдельной latest-таблицы.
Хранить latest и history раздельно.
Обоснование выбора

Хранение только одной версии упрощает модель, но лишает возможности отката и ухудшает работу с конфликтами.
Хранение только истории делает горячий сценарий чтения дороже.
Разделение на latest и versions выбрано потому, что оно:

ускоряет основной read-path;
упрощает разрешение конфликтов;
позволяет хранить историю независимо от горячего контура.
Влияние на проектные решения

Выбранный алгоритм приводит к следующим решениям:

cloud_saves в физической схеме разделяется на две таблицы;
бинарные объекты выносятся в S3/MinIO;
метаданные хранятся в ScyllaDB;
cloud save metadata и blob traffic можно считать и балансировать отдельно;
в межсервисном взаимодействии возможны асинхронные события о конфликте или успешной синхронизации.
7.4 Алгоритм асинхронной обработки покупки
Блок системы

Checkout / Payment Service, Kafka / Redpanda, Library Service, Notification Service, Analytics Service.

Постановка задачи

Необходимо организовать обработку успешной покупки так, чтобы:

платеж фиксировался строго и надежно;
выдача прав владения не была жестко завязана на синхронные вызовы между всеми сервисами;
система выдерживала всплески нагрузки во время акций и распродаж;
отказ одного вторичного сервиса не ломал основную покупку.
Исходные данные, ограничения и требования
...
платежный контур должен оставаться консистентным;
после покупки несколько независимых подсистем должны выполнить побочные действия;
во время распродаж возможны всплески нагрузки.
Последовательность работы алгоритма
Checkout / Payment Service проводит оплату и фиксирует транзакцию в PostgreSQL.
После успешной фиксации публикуется событие order.paid.
Library Service читает событие и добавляет игру пользователю в библиотеку.
Notification Service читает событие и создает уведомление.
Analytics Service читает событие и записывает его в аналитический контур.
При необходимости дополнительные сервисы могут подписываться на то же событие без изменения логики платежного контура.
Используемые структуры данных

Используются:

транзакционные записи в wallet_transactions;
event stream в Kafka / Redpanda;
денормализованная библиотека пользователя;
очередь/топик уведомлений;
аналитические события.
Рассматриваемые альтернативы

Рассматривались следующие варианты:

Полностью синхронный fan-out после покупки.
Асинхронная event-driven обработка через брокер.
Event-driven обработка с outbox-подходом.
Обоснование выбора

Полностью синхронная цепочка делает Checkout Service слишком связным с остальными сервисами и увеличивает latency пользовательского запроса.
Асинхронная публикация событий уменьшает связность и повышает устойчивость.
Для критичных сценариев покупки рекомендуется использовать outbox-подход, чтобы запись транзакции и публикация события были логически согласованы.

Влияние на проектные решения

Выбранный алгоритм приводит к следующим решениям:

между сервисами используется event-driven взаимодействие;
появляются асинхронные паттерны Pub/Sub и Queue/Worker;
обновление библиотеки, уведомлений и аналитики выполняется не в рамках синхронного ответа пользователю;
Kafka / Redpanda становится частью критического межсервисного контура.
...
#### Последовательность работы алгоритма
1. При изменении данных игры в PostgreSQL формируется событие обновления каталога.
2. Search Indexer асинхронно обновляет документ игры в OpenSearch.
3. В индекс записываются не только исходные поля игры, но и денормализованные атрибуты, необходимые для поисковой выдачи.
4. При пользовательском запросе Catalog Service выполняет запрос к OpenSearch.
5. Текстовые поля обрабатываются через inverted index и BM25-подобное ранжирование.
6. Фильтры по жанрам, тегам, цене и статусу применяются как filter-контур без влияния на scoring.
7. Результаты возвращаются в виде уже подготовленных карточек с основными атрибутами для storefront.
...
#### Рассматриваемые альтернативы
Рассматривались следующие варианты:
1. Поиск напрямую в PostgreSQL через ILIKE, GIN, tsvector.
2. Полнотекстовый поиск в PostgreSQL с дополнительной фильтрацией по JSONB.
3. Выделенный поисковый движок OpenSearch.
...
#### Последовательность работы алгоритма
1. Для каждой версии игры формируется manifest.
2. Manifest содержит список chunk-объектов, их идентификаторы, размеры и контрольные суммы.
3. Клиент запрашивает manifest целевой версии.
4. Клиент сравнивает manifest с локальным состоянием.
5. Для отсутствующих или изменившихся chunk-объектов выполняется загрузка с CDN.
6. Chunk-объекты собираются в локальный билд игры.
7. После завершения сборки выполняется проверка контрольных сумм.
...
#### Рассматриваемые альтернативы
Рассматривались следующие варианты:
1. Повторная загрузка игры целиком.
2. Rolling-diff подход, аналогичный rsync.
3. Manifest + chunk-based delivery.
...
#### Последовательность работы алгоритма
При запуске игры
1. Клиент запрашивает latest-метаданные по user_id и game_id.
2. Если локальная версия отсутствует или старее серверной, клиент скачивает актуальный blob.
3. Если локальная и серверная версия совпадают по checksum/version, загрузка не выполняется.

При завершении игры
1. Клиент вычисляет checksum нового сохранения.
2. Blob загружается в object storage.
3. В cloud_saves_versions_by_user_game создается новая версия.
4. В cloud_saves_latest_by_user_game обновляется указатель на актуальное состояние.
5. При обнаружении конфликта версий возможно сохранение обеих версий и последующее разрешение конфликта.
...
#### Рассматриваемые альтернативы
Рассматривались следующие варианты:
1. Хранить только одну актуальную версию.
2. Хранить только журнал версий без отдельной latest-таблицы.
3. Хранить latest и history раздельно.
...
#### Последовательность работы алгоритма
1. Checkout / Payment Service проводит оплату и фиксирует транзакцию в PostgreSQL.
2. После успешной фиксации публикуется событие order.paid.
3. Library Service читает событие и добавляет игру пользователю в библиотеку.
4. Notification Service читает событие и создает уведомление.
5. Analytics Service читает событие и записывает его в аналитический контур.
6. При необходимости дополнительные сервисы могут подписываться на то же событие без изменения логики платежного контура.
...
#### Рассматриваемые альтернативы
Рассматривались следующие варианты:
1. Полностью синхронный fan-out после покупки.
2. Асинхронная event-driven обработка через брокер.
3. Event-driven обработка с outbox-подходом.
...
## 10. Схема проекта
![Схема проекта](img/architecture.svg)
